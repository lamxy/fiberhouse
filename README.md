...
| 范围 | 状态 | 说明 |
|---|---|---|
| Fiber HTTP、Provider 主链、配置与日志、JSON 响应、校验 | 已接入 | 已有明确入口和可达的运行时实现；具体能力可能仍需使用方应用完成注册、配置或依赖注入 |
| Gin、GlobalManager 生命周期、L2 缓存、任务、CLI、数据库、二进制响应 | 实验性 | 已有可运行实现和调用路径，但生命周期、错误处理、并发或外部依赖边界尚未充分收敛；采用前需评估限制，并由应用显式管理相关风险 |
