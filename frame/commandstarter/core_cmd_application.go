...
func (ccc *CoreCmdCli) RegisterGlobalErrHandler(fca frame.FrameCmdStarter) {
	...
	// 注册全局错误处理器
	fca.GetApplication().(frame.ApplicationCmdRegister).RegisterGlobalErrHandler(ccc.coreApp)
}
...
func (ccc *CoreCmdCli) RegisterCommands(fca frame.FrameCmdStarter) {
	...
	fca.GetApplication().(frame.ApplicationCmdRegister).RegisterCommands(ccc.coreApp)
}
...
func (ccc *CoreCmdCli) RegisterCoreGlobalOptional(fca frame.FrameCmdStarter) {
	...
	// 注册应用核心的全局可选项
	fca.GetApplication().(frame.ApplicationCmdRegister).RegisterCoreGlobalOptional(ccc.coreApp)
}
