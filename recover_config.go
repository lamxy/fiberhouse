...
/*
recover中间件配置模块 RecoverConfig，定义中间件的配置结构和默认值。
...
使用默认配置：IErrorHandler 为 interface 接口

	app.Use(IErrorHandler.RecoverMiddleware())

使用自定义配置：

	app.Use(IErrorHandler.RecoverMiddleware(recover.Config{
...
type RecoverConfig struct {
	...
	// JsonCodec 用于将数据编码为 JSON 格式的函数
	JsonCodec func(interface{}) ([]byte, error)

	// Stdout 默认输出目标是 os.Stdout
	Stdout bool

	// DebugMode 调试模式：true 将详细错误信息响应给客户端，否则仅记入日志
	DebugMode bool
}
