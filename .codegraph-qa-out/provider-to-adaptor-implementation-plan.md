...
- [x] **Step 1: 確認 Task 1 的精確舊 import 集合**
...
- [x] **Step 3: 更新全部 context import、alias 與 selector**
...
- [x] **Step 1: 搬移 error handler package**
...
- [x] **Step 2: 對齊 error handler package clause**
...
- [x] **Step 3: 更新兩個 starter import 與 selector**
...
- [x] **Step 4: 格式化並驗證 error handler 搬移**
...
- [x] **Step 5: 審閱並提交 error handler 搬移**
...
- [x] **Step 2: 驗證新 package、package clause 與 selector**
...
- [x] **Step 2: 查詢新 error handler 路徑與 starter caller**
...
- Task 1 完成於 commit `1140436`：三個 context 定義檔為 100% Git rename，19 個舊 context import 全部切換到 `adaptor/context`，alias 統一為 `adaptorctx`。
- Task 2 完成於 commit `ec30c9b`：兩個 error handler 檔搬至 `adaptor/errorhandler`，package clause 改為 `errorhandler`，兩個 starter 使用 `adaptorerrorhandler`；`FiberErrorHandler` 與 `GinErrorHandler` 均保留原名。
- ast-grep 與 ripgrep 對兩個舊 Go import path 均無匹配。
- `go list ./adaptor/...` 只列出 `github.com/lamxy/fiberhouse/adaptor/context` 和 `github.com/lamxy/fiberhouse/adaptor/errorhandler`。
- `go build ./...` 與 `go build -o /tmp/fiberhouse-adaptor-rename-fhweb ./example_main/main.go` 均成功。
- `go test ./...` 只重現改名前基線的 `bootstrap`、`component/writer` 失敗；沒有新增失敗 package，也沒有 package not found、undefined selector 或 import cycle。
- CodeGraph CLI 明確警告目前結果來自主工作樹 `/mnt/d/code/github_opensource/tmp/fiberhouse`，不是本 worktree，因此仍顯示舊 source path。依計畫不手工修改或重建 `.codegraph` 生成資料；本次以工作樹內 AST 掃描、Go package discovery 和 build 結果為準。
- `README.md`、`docs/*.md`、`.codegraph/`、`graphify-out/` 與歷史日誌均未修改。
