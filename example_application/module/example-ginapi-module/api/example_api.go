...
func (h *ExampleHandler) HelloWorld(c *gin.Context) {
	...
	// 类型断言失败响应
	_ = c.Error(fmt.Errorf("type assert failed for TestService: %v", ts))
}
...
func (h *ExampleHandler) GetExample(c *gin.Context) {
	// 获取语言
	var lang = c.GetHeader(constant.XLanguageFlag)
	if lang == "" {
		lang = "en"
	}
	...
	if errVw := vw.GetValidate(lang).Struct(objId); errVw != nil {
		...
		if errors.As(errVw, &errs) {
			_ = c.Error(vw.Errors(errs, lang, true))
			return
		}
	}
...
func (h *ExampleHandler) GetExamples(c *gin.Context) {
	...
	if errsMap := vw.GetValidate(lang).ValidateMap(vaMap, vaRule); len(errsMap) > 0 {
		_ = c.Error(vw.ErrorsMap(errsMap, lang, true))
		return
	}
