...
func (h *CommonHandler) TestGetInstance(c *gin.Context) {
	...
	if ts, ok := testService.(*service.TestService); ok {
		_ = fiberhouse.Response().SuccessWithData(t+":"+ts.HelloWorld()).SendWithCtx(adaptorctx.WithGinContext(c), http.StatusOK)
		return
	}

	_ = fiberhouse.Response().SuccessWithData(t).SendWithCtx(adaptorctx.WithGinContext(c), http.StatusOK)
}
...
func (h *CommonHandler) TestGetMustInstance(c *gin.Context) {
	...
	_ = fiberhouse.Response().SuccessWithData(t+testService.HelloWorld()).SendWithCtx(adaptorctx.WithGinContext(c), http.StatusOK)
}
...
func (h *CommonHandler) TestGetMustInstanceFailed(c *gin.Context) {
	...
	defer func() {
		if r := recover(); r != nil {
			_ = c.Error(fmt.Errorf("get instance failed: %v", r))
		}
	}()
