...
func (m *Module) RegisterModuleMiddleware(cs frame.CoreStarter) {
	// 注册模块(子系统)级中间件
	moduleApi.RegisterMiddleware(cs.GetCoreApp().(*fiber.App))
}

// RegisterModuleRouteHandlers 注册模块(子系统)级路由处理器
func (m *Module) RegisterModuleRouteHandlers(cs frame.CoreStarter) {
	// 注册各模块中间件和路由处理器
	RegisterRouteHandlers(m.Ctx, cs.GetCoreApp().(*fiber.App))
}

// RegisterSwagger 注册swagger
func (m *Module) RegisterSwagger(cs frame.CoreStarter) {
	// 注册swagger
