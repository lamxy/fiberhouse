...
| Gin | 无 TLS 配置时调用 `http.Server.ListenAndServe()`；已加载 TLS 配置时调用 `ListenAndServeTLS("", "")` | 最多等待 30 秒 `Shutdown`，随后清空容器并关闭日志器 | TLS 证书加载已有单元/契约测试，但没有真实 listener/握手集成验证；应用状态同样在 server 返回后才写入 |
