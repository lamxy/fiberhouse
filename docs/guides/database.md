...
标准 HTTP shutdown 调用 `ClearAll(true)`，不会逐项 `Close`。GlobalManager `Release` 又有 `atomic.Value.Store(nil)` 的已知 panic 风险，因此数据库创建者应显式关闭 client，不依赖容器清空；详见[《GlobalManager》](global-manager.md)。
