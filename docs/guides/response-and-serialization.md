...
所有发送方法都返回 `error`。JSON marshal、MsgPack/Protobuf 编码或底层 writer 失败会沿返回值传播，但现有上层路径并不完全一致：panic recovery 忽略发送结果；Gin 错误 adaptor 会把错误处理器返回的发送错误再次 panic；Fiber adaptor 在成功写出统一错误响应后仍可能把原始 handler error 返回给 Fiber。最后一种是“已写响应后继续传播原错误”的静态风险，尚未核对 Fiber 对最终 socket 的每一种处理结果。
