...
Redis 实际读取连接地址、认证、DB、pool 和超时配置，包括 `host`、`port`、`password`、`db`、`poolSize`、`minIdleConns`、`dialTimeout`、`readTimeout`、`writeTimeout`、`poolTimeout`、`connMaxIdleTime`、`connMaxLifetime`。这些时间数值会乘以 `time.Second`。
...
只有 `cache.redis.protection.enable=true` 时，`NewRedisDb` 才向 GlobalManager 注册默认 `shardedBloomFilter` 和 `wrapCircuitBreaker` initializer，并按 `protection.type.*.selected` 取得实现。随后还必须在每次 `CacheOption` 上开启对应开关，保护逻辑才会运行。

- singleflight：合并 Redis `Get`，不合并 read-through loader。
- Bloom filter：filter 判定“不存在”时仍尝试一次 Redis；Redis miss 会转成 `ErrRejectedByBloomFilter`，使 `GetCached` 不再执行 loader。新合法 key 的冷启动语义需要应用自行验证。
- circuit breaker：只包裹 Redis `Get`；`Set` 中的 breaker 分支当前被注释，写入不受保护。打开时 `GetCached` 可走一个 fallback。
...
Local metrics 只有在 `cache.local.metrics=true` 时可用。`GetMetricsInfo` 汇总 hit/miss、eviction、drop 等计数；L2 还暴露 local metrics 和 pool capacity/running/free。L2 的 local metrics 方法直接把 local 断言为 `*cachelocal.LocalCache`，替换自定义 local 实现时可能 panic。
