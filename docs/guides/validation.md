...
FiberHouse 的 [`component/validate`](../../component/validate/) 包装 go-playground/validator，为每种启用语言保存独立 validator 与 translator，并把 `validator.ValidationErrors` 转成框架的 `ValidateException`。包装器在创建 `AppContext`/`CmdContext` 时初始化，应用随后可在 Web 启动阶段追加语言、tag 和 translation。
...
`AppContext`、`CmdContext` 各自在构造时持有 wrapper，但它们仍可能共享配置、GlobalManager 与异常表的进程级对象。测试多语言或自定义 tag 时，应避免并行修改同一 wrapper，并为异常表初始化提供隔离。
